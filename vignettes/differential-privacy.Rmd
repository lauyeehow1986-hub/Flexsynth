...
The remaining honest limitation is that transitions are per-variable:
cross-variable coupling is carried at `t = 1` by the initial-state tree, not
re-measured at each later step.
...
- `baseline` (columns held exactly constant within a person) applies to a flat
  longitudinal `synth()` release; holding a longitudinally-modelled *linked* child
  table's columns constant is not yet supported.
