...
Two honest limitations of this first-order model: every non-index column is
treated as time-varying (a genuinely constant baseline covariate may therefore
drift slightly between a synthetic person's visits), and transitions are
per-variable (cross-variable coupling is carried at `t = 1` by the initial-state
tree, not re-measured at each later step).
...
Two first-cut limitations under DP: child variables are modelled by their own
within-table marginals (cross-table statistical coupling — the parent→child
conditioning that Track A preserves — is *not* retained under DP, only referential
integrity), and within-table longitudinal (Markov) structure is not modelled for a
linked table. Constraints are refused, as elsewhere under DP.

## What Track B does *not* do (yet)

- **Cross-table conditioning** under linked DP: only referential integrity and
  per-table marginals are preserved, not the parent→child statistical coupling.
...
- Publicly-declared baseline columns (held exactly constant within a person
  under DP) are not yet supported — see the longitudinal note above.
